...
def albaranes_vs_facturas(request):
    ...
    sql1 = """select  TO_CHAR(alc.FECHA, 'YYYY-MM-DD') FECHA, 
                    ...
                    alc.CODIGO_ALMACEN,
                    (SELECT SUM(lin.importe_lin_neto_div) from albaran_compras_l lin where lin.NUMERO_DOC_INTERNO = alc.NUMERO_DOC_INTERNO) AS IMPORTE_LIN_NETO_DIV_USD,
                    (SELECT MAX(lin.cambio)               from albaran_compras_l lin where lin.NUMERO_DOC_INTERNO = alc.NUMERO_DOC_INTERNO) AS CAMBIO,
                    (SELECT SUM(lin.importe_lin_neto)     from albaran_compras_l lin where lin.NUMERO_DOC_INTERNO = alc.NUMERO_DOC_INTERNO) AS IMPORTE_LIN_NETO_EUR
            ...
            where alc.centro_contable = '01'
              ...
              -- and alc.CODIGO_PROVEEDOR = '000718'
    ...
    for o2 in out:
        sql2 = """SELECT DISTINCT fl.NUMERO_FACTURA,
                    ...
                    (SELECT fcc.LIQUIDO_FACTURA_DIV FROM FACTURAS_COMPRAS_CAB fcc where fcc.numero_factura = fl.numero_factura) AS LIQUIDO_FACTURA_DIV,
                    (SELECT fcc.VALOR_CAMBIO        FROM FACTURAS_COMPRAS_CAB fcc where fcc.numero_factura = fl.numero_factura) AS VALOR_CAMBIO,
                    (SELECT fcc.LIQUIDO_FACTURA     FROM FACTURAS_COMPRAS_CAB fcc where fcc.numero_factura = fl.numero_factura) AS LIQUIDO_FACTURA

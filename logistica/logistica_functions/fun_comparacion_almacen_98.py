...
def get_expediente_almacen_98(request, oracle, valor_cambio):
    ...
    sql = """SELECT c.NUMERO_DOC_EXT,
                    ...
                    (SELECT SUM(NVL(li.IMPORTE_LIN_NETO_DIV, li.IMPORTE_LIN_NETO))
                        FROM ALBARAN_COMPRAS_L li
                        WHERE li.NUMERO_DOC_INTERNO = c.NUMERO_DOC_INTERNO AND li.CODIGO_EMPRESA = c.CODIGO_EMPRESA) AS IMPORTE_TOTAL_ORIGINAL,
...
def get_textos(request, oracle, num_doc_ext, valor_cambio):
    sql = """SELECT 
                    ...
                    (SELECT SUM(NVL(lai.IMPORTE_LIN_NETO_DIV, lai.IMPORTE_LIN_NETO))
                        FROM ALBARAN_COMPRAS_L lai
                        WHERE lai.NUMERO_DOC_INTERNO = acc.NUMERO_DOC_INTERNO AND lai.CODIGO_EMPRESA = acc.CODIGO_EMPRESA) AS IMPORTE_TOTAL_ORIGINAL,

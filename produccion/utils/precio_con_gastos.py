def consultar_precio_con_gastos(oracle, exp_id, art_code, cantidad):
    sql = """SELECT  articulos.codigo_familia c1,articulos.d_codigo_familia c2,expedientes_articulos_embarque.articulo c3,expedientes_articulos_embarque.d_articulo c4,expedientes_hojas_seguim.num_expediente n1,expedientes_hojas_seguim.num_hoja n2,expedientes_articulos_embarque.contenedor c5,expedientes_imp.forma_envio c6,expedientes_articulos_embarque.cantidad n3,(CASE
              ...
              END
               ...
               n4,expedientes_articulos_embarque.presentacion c7,expedientes_articulos_embarque.destino_especial c8,expedientes_articulos_embarque.deposito_aduanero c9,expedientes_articulos_embarque.precio n5,expedientes_articulos_embarque.importe n6,expedientes_hojas_seguim.tipo_cambio c10,coalesce(EXPEDIENTES_HOJAS_SEGUIM.VALOR_CAMBIO, EXPEDIENTES_IMP.VALOR_CAMBIO,1) n7,1 / coalesce(EXPEDIENTES_HOJAS_SEGUIM.VALOR_CAMBIO, EXPEDIENTES_IMP.VALOR_CAMBIO,1) n8,DECODE(coalesce(EXPEDIENTES_HOJAS_SEGUIM.VALOR_CAMBIO, EXPEDIENTES_IMP.VALOR_CAMBIO,1), 
                   ...
                   and hs.codigo_articulo = expedientes_articulos_embarque.articulo) / DECODE(articulos.unidad_valoracion, 1, expedientes_articulos_embarque.cantidad_unidad1, 2, expedientes_articulos_embarque.cantidad_unidad2)) + (expedientes_articulos_embarque.precio *
                      DECODE(expedientes_hojas_seguim.tipo_cambio, 'E', DECODE(expedientes_imp.cambio_asegurado, 'S', expedientes_imp.valor_cambio, 'N', 1), 'S', expedientes_hojas_seguim.valor_cambio, 'N',coalesce(EXPEDIENTES_HOJAS_SEGUIM.VALOR_CAMBIO, EXPEDIENTES_IMP.VALOR_CAMBIO,1)))) n10,expedientes_imp.d_clave_arancel c11,expedientes_hojas_seguim.d_situacion_logistica c12,expedientes_hojas_seguim.documentacion_x_contenedor c13,expedientes_hojas_seguim.fecha_prev_llegada c14,expedientes_hojas_seguim.fecha_prev_embarque c15,expedientes_imp.d_plantilla c16,NVL(EXPEDIENTES_HOJAS_SEGUIM.PROVEEDOR,EXPEDIENTES_IMP.PROVEEDOR) c17,expedientes_hojas_seguim.d_proveedor_hoja c18,expedientes_imp.d_descripcion_expediente c19,expedientes_hojas_seguim.codigo_entrada c20,expedientes_hojas_seguim.id_seguro_cambio n11,(select nombre from bancos b, Seguros_cambio c
                      where b.codigo_rapido = c.banco
                      and b.empresa = c.empresa
                      and c.id = expedientes_hojas_seguim.id_seguro_cambio) c21,(SELECT MAX(DOCUMENTO) FROM SEGUROS_CAMBIO c WHERE c.ID = expedientes_hojas_seguim.id_seguro_cambio) c22,(select SUM(IMPORTE) from  Seguros_cambio c
                      where  c.id = expedientes_hojas_seguim.id_seguro_cambio) n12,expedientes_hojas_seguim.carta_credito c23,(SELECT MAX(FECHA_VENCIMIENTO) FROM CARTAS_CREDITO C
              WHERE C.NUMERO_CARTA_CREDITO = expedientes_hojas_seguim.CARTA_CREDITO
              ) c24,CASE WHEN EXPEDIENTES_HOJAS_SEGUIM.D_SITUACION_LOGISTICA ='CONTRATO' THEN '0:AMARILLO' ELSE '0:LIMA' END gi$color   FROM (SELECT ARTICULOS.*,DECODE(articulos.codigo_familia,NULL,NULL,(SELECT lvfm.descripcion FROM familias lvfm WHERE lvfm.codigo_familia = articulos.codigo_familia AND lvfm.numero_tabla = 1 AND lvfm.codigo_empresa = articulos.codigo_empresa)) D_CODIGO_FAMILIA FROM ARTICULOS) ARTICULOS,(SELECT EXPEDIENTES_IMP.*,DECODE(expedientes_imp.clave_arancel,NULL,NULL,(SELECT lvarimp.descripcion FROM aranceles_imp lvarimp WHERE lvarimp.clave_arancel = expedientes_imp.clave_arancel AND lvarimp.codigo_empresa = expedientes_imp.empresa)) D_CLAVE_ARANCEL,DECODE(expedientes_imp.plantilla,NULL,NULL,(SELECT lvpltimp.nombre FROM plantillas_impor lvpltimp WHERE lvpltimp.codigo = expedientes_imp.plantilla AND lvpltimp.empresa = expedientes_imp.empresa)) D_PLANTILLA,(SELECT lvexpc.descripcion FROM expedientes_cab lvexpc WHERE lvexpc.codigo = expedientes_imp.codigo AND lvexpc.empresa = expedientes_imp.empresa) D_DESCRIPCION_EXPEDIENTE FROM EXPEDIENTES_IMP) EXPEDIENTES_IMP,(SELECT EXPEDIENTES_HOJAS_SEGUIM.*,(SELECT DESCRIPCION FROM EXPEDIENTES_HOJAS_SITUACION WHERE CODIGO=EXPEDIENTES_HOJAS_SEGUIM.SITUACION_LOGISTICA) D_SITUACION_LOGISTICA,NVL((SELECT pr.nombre
               FROM proveedores pr
              ...
              AND expedientes_hojas_seguim.num_expediente = :exp_id and expedientes_articulos_embarque.articulo = :art_code"""
